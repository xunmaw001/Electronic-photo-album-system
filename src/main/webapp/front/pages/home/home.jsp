...
<body>
  <div id="app">
    ...
    <div class="part2" style="margin-top: 50px;">
      <div class="part2_1">
        ...
        <div class="more">
            <a href="#" @click="jump('../xiangche/list.jsp')">MORE>></a>
        </div>
      </div>
      <div class="part2_2">
        <div class="caseBox" v-for="(item,index) in youxizhongxinRecommend" v-bind:key="index">
          <div class="caseBox_1">
            <div class="caseImg" style="background-size: cover;background-repeat: no-repeat;background-position: center;"><img :src="item.imgPhoto?item.imgPhoto.split(',')[0]:''" width="390" height="320" /></div>
            <div class="hoverbg" style="display: none;">
                <a :href="'../xiangche/list.jsp?id='+item.id">查看相册图片</a>
            </div>
          </div>
          <div class="caseBox_2" style="text-align: center;">
            <a :href="'../xiangche/list.jsp?id='+item.id">{{item.xcname}}</a>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="part2" style="margin-top: 50px;">
      ...
      <div class="part2_2">
        <div class="caseBox" v-for="(item,index) in youxizhongxinList" v-bind:key="index">
          <div class="caseBox_1">
            <div class="caseImg" style="background-size: cover;background-repeat: no-repeat;background-position: center;"><img :src="item.picture" width="390" height="320" /></div>
            <div class="hoverbg" style="display: none;">
                <a :href="'../news/detail.jsp?id='+item.id">查看详情</a>
            </div>
          </div>
          <div class="caseBox_2">
            <a :href="'../news/detail.jsp?id='+item.id">{{item.title}}</a>
          </div>
        </div>
      </div>
    </div>
  ...
  <script>
    ...
    layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery'], function() {
      ...
      http.request(`xiangche/list`, 'get', {
        page: 1,
        limit: 6
      }, function(res) {
